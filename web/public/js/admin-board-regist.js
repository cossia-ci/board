$('#frms').validate({
    rules: {
        ...
        'list[thumb][width]' : {
            required: function(){
                return  ($('input[name="type"]:checked').val() == 'gallery' || $('input[name="type"]:checked').val() == 'event' ) ? true : false;
            },
            min: 1
        },
        'list[thumb][height]' : {
...
$(function(){
    ...
    $(document).on('click', '.btn-gray', function(){
        ...
        bootbox.confirm({
            title: '선택하세요',
            message: level_table.co_split( {tbody: tbody, target: $(this).data('target')} ),
            buttons: {
                confirm: {
                    label: '선택완료',
                    className: 'btn-gray'
