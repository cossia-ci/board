...
//    protected $beforeUpdate         = [];
    ...
    public function getData( int $ano = 0 ) : array
    {
        return match( $ano ){
            0 => $this->findAll(),
            default => $this->find( $ano )
        };
    }

    public function setWhere() : mixed
    {
        if( isset($_GET['keyword']) && $_GET['keyword'] ) $this->like( $_GET['key'], $_GET['keyword'] );
    ...
    {
        return [
            'data' => $this->paginate( $_GET['perPage'] ),
            'page' => $this->pager,
        ];
    }
            ...
            foreach($data['data'] as $key => $val)
                if( is_array($val) ){
                    foreach( $val as $k_ => $v_ ){
                        if( in_array($k_, $this->jsonField) && $v_ ) $data['data'][$key][$k_] = json_decode($v_, true);
                    }
                }else{
                    if( in_array($key, $this->jsonField) && $val ) $data['data'][$key] = json_decode($val, true);
                }
        return $data;
    }
