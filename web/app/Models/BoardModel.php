            ...
            foreach($data['data'] as $key => $val)
                if( is_array($val) ){
                    foreach( $val as $k_ => $v_ ){
                        if( in_array($k_, $this->jsonField) && $data['data'][$key][$k_] ) $data['data'][$key][$k_] = json_decode($v_, true);
                    }
                }else{
                    if( in_array($key, $this->jsonField) && $data['data'][$key] ) $data['data'][$key] = json_decode($val, true);
                }
        return $data;
    }
