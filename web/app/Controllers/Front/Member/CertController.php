...
class CertController extends \App\Controllers\Front\Controller
{
	public function logIn()
	{
        ...
        $this->setData( 'social', get_config_data('config', 'social') );
	}
    ...
    public function JoinUs()
    {
        ...
        $this->setData('privacy', get_config_data('infos','privacy')['data'] );
        $this->setData('tos', get_config_data('infos','tos')['data'] );
        $this->setData('agreement', get_config_data('infos','agreement')['data'] );
    ...
    {
        $model = new BaseModel('co_member');
        $data = match( $_POST['type'] ){
            'id' => $model->where('JSON_EXTRACT(`info`, "$.name") = "'.$_POST['name'].'"', null, false)
                        ->where('JSON_EXTRACT(`info`, "$.phone") = "'.$_POST['phone'].'"', null, false)
                        ->first(),
            'pw' => $model->where('JSON_EXTRACT(`info`, "$.name") = "'.$_POST['name'].'"', null, false)
                        ->where('JSON_EXTRACT(`info`, "$.phone") = "'.$_POST['phone'].'"', null, false)
                        ->where('id', $_POST['id'])
                        ->first()
        };
