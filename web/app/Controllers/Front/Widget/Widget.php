...
class Widget
{
    public static function getBoradSome( string $code, int $limit = 5, bool|string $skin = 'default' )
    {
        $model = new BoardModel($code);
        $data = $model->setLimt($limit)->getList();
        if( $skin === true ) return $data['data'];
        else echo view('Front/widget/board_'.$skin.'.html', ['data'=>$data['data'], 'code'=>$code]);
    }
