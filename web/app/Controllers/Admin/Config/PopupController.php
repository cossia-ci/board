...
class PopupController extends \App\Controllers\Admin\Controller
{
    public function list()
    {
        ...
        if(isset($_GET['sDate'])&&$_GET['sDate']) {
            match( $_GET['key'] ){
                'regDt'     =>  $model->where('DATE(regDt) >= ', $_GET['sDate']),
                'viewDt'    =>  $model->where('DATE(sDate) <= ', $_GET['sDate']),
            };
            # switch($_GET['key']){
        ...
        if(isset($_GET['eDate'])&&$_GET['eDate']) {
            match( $_GET['key'] ){
                'regDt'     =>  $model->where('DATE(regDt) <= ', $_GET['eDate']),
                'viewDt'    =>  $model->where('DATE(eDate) >= ', $_GET['eDate']),
            };
            # switch($_GET['key']){
